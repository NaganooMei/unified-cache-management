...
默认路径仍然是 CE。只有编译时打开 `UCM_ENABLE_ASCEND_FFTS_PIPELINE`，并且运行时配置 `cache_h2d_transport` 为 `ffts_pipeline`，LoadQueue 才会选择 FFTS executor。
...
打开后，Ascend trans 组件会查找 FFTS header 和 `libruntime`，并把 `ascend_h2d_ffts_pipeline.cc`、`ffts_d2d_dispatcher.cc` 加入编译。如果缺少依赖，CMake 会直接失败。
