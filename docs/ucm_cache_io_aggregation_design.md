...
- 不新增 connector 侧 load / dump trace 日志，性能观测优先复用 CacheStore metrics。
...
正式配置只接受 `cache_io_aggregation`。底层 Ascend FFTS 只作为聚合 executor 的实现细节，不作为用户可见的运行时模式名。
...
`MakeCacheIOExecutor` 根据 `cache_io_aggregation` 选择执行器：
...
默认关闭。开启后，Ascend transfer 组件会检查 FFTS 相关 header 和 runtime library，并编译聚合搬运所需的 Ascend 实现。关闭后，默认逐 tensor 路径仍可构建和运行，但运行时不能启用 `cache_io_aggregation`。
...
正式实现不在 connector 侧新增 load / dump trace 日志，避免服务日志被逐请求或逐 rank 的搬运耗时刷屏。性能观测优先复用 CacheStore 已有 metrics，例如 load queue wait、backend wait、H2D duration、dump queue wait、D2H duration、backend submit duration 以及 TransManager 侧的任务耗时和带宽指标。
...
| metrics | 确认 load / dump 搬运耗时和带宽指标正常更新 |
