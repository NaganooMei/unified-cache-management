...
/*
 * If you need high performance, enable the SECUREC_WITH_PERFORMANCE_ADDONS macro, default is
 * enable. The macro is automatically closed on the windows platform and linux kernel
 */
...
/* On windows platform, can't use optimized function for there is no __builtin_constant_p like
 * function */
...
/*
 * Add  the -DSECUREC_SUPPORT_FORMAT_WARNING  compiler option to support  -Wformat.
...
/*
 * This is a built-in function that can be used without a declaration, if you encounter an
 * undeclared compilation alarm, you can add -DSECUREC_NEED_BUILTIN_EXPECT_DECLARE to compiler
 * options
 */
    ...
    (((void *)(dest) != NULL && (void *)(src) != NULL && (size_t)(destMax) > 0 &&              \
      ...
      (SECUREC_TWO_MIN((size_t)(count), strlen(src)) + 1) <= (size_t)(destMax))                \
         ? (((size_t)(count) < strlen(src))                                                    \
                ? (memcpy((dest), (src), (count)), *((char *)(dest) + (count)) = '\0', EOK)    \
...
#define SECUREC_STRCAT_SM(dest, destMax, src)                                           \
    ({                                                                                  \
        ...
        if ((void *)(dest) != NULL && (void *)(src) != NULL && (size_t)(destMax) > 0 && \
            ...
            if (catRet != EOK) { catRet = strcat_s((dest), (destMax), (src)); }         \
        } else {                                                                        \
...
#define SECUREC_STRNCAT_SM(dest, destMax, src, count)                                              \
    ({                                                                                             \
        ...
        if ((void *)(dest) != NULL && (void *)(src) != NULL && (size_t)(destMax) > 0 &&            \
            ...
            if (ncatRet != EOK) { ncatRet = strncat_s((dest), (destMax), (src), (count)); }        \
        } else {                                                                                   \
